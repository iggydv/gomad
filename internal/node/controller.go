...
func (c *Controller) transitionToSuperPeer(ctx context.Context, permanent bool, groupName string) error {
	...
	if oldPeer != nil {
		// Deactivate preserves local storage so objects from the peer phase
		// survive the transition and remain available under the SP role.
		oldPeer.Deactivate()
	}
	if oldSP != nil {
	...
	// gsHost is passed so HandleJoin can notify each joining peer that the SP
	// is a valid group-storage target (enabling peer→SP replication).
	sp.TakeLeadership(groupName, !permanent, c.gsAddr)
...
func (g *gatedGroupStorageHandler) Put(obj *pbm.GameObjectGrpc) (bool, error) {
	g.warnIfElecting()
	ok, err := g.ctrl.peerStore.Put(obj, false, false)
	if ok && err == nil {
		g.ctrl.stateMu.RLock()
		isSP := g.ctrl.state.IsActingSuperPeer()
		gsAddr := g.ctrl.gsAddr
		g.ctrl.stateMu.RUnlock()
		// When acting as SP, record our own copy in the group ledger so the
		// repair scheduler counts us as a valid replica, preventing spurious
		// repair requests for objects already held on the SP.
		if isSP && gsAddr != "" {
			g.ctrl.groupLedger.AddToObjectLedger(obj.Id, ledger.MetaData{ID: gsAddr, TTL: 0})
			g.ctrl.groupLedger.AddToPeerLedger(gsAddr, ledger.MetaData{ID: obj.Id, TTL: 0})
		}
	}
	return ok, err
}
