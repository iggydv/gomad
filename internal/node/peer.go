...
// Deactivate tears down this peer's group connections without erasing local
// storage. Use instead of Close when transitioning to the super-peer role so
// that locally stored objects survive the role change and are counted toward
// the replication factor in the new group.
//
// Unlike Close, Deactivate does NOT call TruncateLocal or NotifyAllPeersRemovePeer
// because the node is staying in the network (just changing role), and its
// GroupStorageService address remains valid for incoming replication traffic.
func (p *Peer) Deactivate() error {
	p.mu.Lock()
	defer p.mu.Unlock()
	if !p.active {
		return nil
	}
	// Close the (likely dead) super-peer connection.
	if p.superPeerClient != nil {
		p.superPeerClient.Close()
		p.superPeerClient = nil
	}
	// Release outbound group-storage connections; the SP role manages peers
	// via PeerService, not GroupStorageService clients.
	p.peerStorage.GroupStorage().CloseClients()
	// Clear the group ledger — the new SP role will build its own from
	// incoming HandleJoin calls.
	p.groupLedger.ClearAll()
	// NOTE: local Pebble storage is intentionally NOT truncated here. Objects
	// stored during the peer phase remain available for the SP's repair
	// scheduler and for direct GroupStorage reads after the transition.
	p.active = false
	return nil
}
